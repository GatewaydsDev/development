<?php

use App\Models\UserLevel;

return [
    'permissions' => [
        'view-dashboard' => [
            'name' => 'View dashboard',
            'group' => 'Pages',
            'description' => 'Access the authenticated dashboard page.',
        ],
        'manage-profile' => [
            'name' => 'Manage profile',
            'group' => 'Account',
            'description' => 'Open and update the signed-in user profile.',
        ],
        'view-users' => [
            'name' => 'See all users',
            'group' => 'Users',
            'description' => 'Open the users list page.',
        ],
        'create-users' => [
            'name' => 'Add new users',
            'group' => 'Users',
            'description' => 'Open the add user page and create users.',
        ],
        'update-users' => [
            'name' => 'Update users',
            'group' => 'Users',
            'description' => 'Open edit user pages and save user updates.',
        ],
        'view-company' => [
            'name' => 'View company',
            'group' => 'Company',
            'description' => 'Open the company profile page.',
        ],
        'manage-access' => [
            'name' => 'Manage access control',
            'group' => 'Access Control',
            'description' => 'Grant or deny user level permissions.',
        ],
        'manage-notifications' => [
            'name' => 'Manage notifications',
            'group' => 'Notifications',
            'description' => 'Open, update, and delete dashboard notifications.',
        ],
        'view-projects' => [
            'name' => 'See all projects',
            'group' => 'Projects',
            'description' => 'Open the projects list and project detail pages.',
        ],
        'create-projects' => [
            'name' => 'Add new projects',
            'group' => 'Projects',
            'description' => 'Open the add project page and create projects.',
        ],
        'update-projects' => [
            'name' => 'Update projects',
            'group' => 'Projects',
            'description' => 'Open edit project pages and save project updates.',
        ],
        'delete-projects' => [
            'name' => 'Delete projects',
            'group' => 'Projects',
            'description' => 'Remove project records.',
        ],
        'view-customers' => [
            'name' => 'See all customers',
            'group' => 'Customers',
            'description' => 'Open the customers list page.',
        ],
        'create-customers' => [
            'name' => 'Add new customers',
            'group' => 'Customers',
            'description' => 'Open the add customer page and create customers.',
        ],
        'update-customers' => [
            'name' => 'Update customers',
            'group' => 'Customers',
            'description' => 'Open edit customer pages and save customer updates.',
        ],
        'delete-customers' => [
            'name' => 'Delete customers',
            'group' => 'Customers',
            'description' => 'Remove customer records.',
        ],
        'view-employees' => [
            'name' => 'See all employees',
            'group' => 'Employees',
            'description' => 'Open the employees list page.',
        ],
        'create-employees' => [
            'name' => 'Add new employees',
            'group' => 'Employees',
            'description' => 'Open the add employee page and create employees.',
        ],
        'update-employees' => [
            'name' => 'Update employees',
            'group' => 'Employees',
            'description' => 'Open edit employee pages and save employee updates.',
        ],
        'delete-employees' => [
            'name' => 'Delete employees',
            'group' => 'Employees',
            'description' => 'Remove employee records.',
        ],
    ],

    'defaults' => [
        UserLevel::SUPER_ADMIN => [
            'view-dashboard',
            'manage-profile',
            'view-users',
            'create-users',
            'update-users',
            'view-company',
            'manage-access',
            'manage-notifications',
            'view-projects',
            'create-projects',
            'update-projects',
            'delete-projects',
            'view-customers',
            'create-customers',
            'update-customers',
            'delete-customers',
            'view-employees',
            'create-employees',
            'update-employees',
            'delete-employees',
        ],
        UserLevel::ADMINISTRATOR => [
            'view-dashboard',
            'manage-profile',
            'view-company',
            'view-projects',
            'create-projects',
            'update-projects',
            'delete-projects',
            'view-customers',
            'create-customers',
            'update-customers',
            'delete-customers',
            'view-employees',
            'create-employees',
            'update-employees',
            'delete-employees',
        ],
        UserLevel::ADMIN => [
            'view-dashboard',
            'manage-profile',
            'view-company',
            'view-projects',
            'create-projects',
            'update-projects',
            'view-customers',
            'create-customers',
            'update-customers',
        ],
        UserLevel::PROJECT_MANAGER => [
            'view-dashboard',
            'manage-profile',
            'view-projects',
            'update-projects',
            'view-customers',
            'create-customers',
            'update-customers',
        ],
        UserLevel::USER => [
            'view-dashboard',
            'manage-profile',
        ],
        UserLevel::VISITOR => [
            'manage-profile',
        ],
    ],
];
